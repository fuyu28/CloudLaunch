...
`
                ...
                // タイムスタンプが同じ場合
                syncResult.skipped++
                syncResult.details.push(`スキップ: ${localMemo.title} (同じタイムスタンプ)`)
          ...
          try {
            ...
            if (existingMemo) {
              ...
              if (cloudLastModified > localUpdatedAt) {
                ...
                syncResult.details.push(`ローカル更新: ${cloudMemo.memoTitle} (クラウド版が新しい)`)
...
`
                ...
                await r2Client.send(uploadCommand)
                syncResult.cloudOverwritten++
                syncResult.details.push(`クラウド更新: ${existingMemo.title} (ローカル版が新しい)`)
              } else {
                // タイムスタンプが同じ場合はスキップ
                syncResult.skipped++
                syncResult.details.push(`スキップ: ${cloudMemo.memoTitle} (同じタイムスタンプ)`)
              }
            } else {
              // 新しいメモを作成
