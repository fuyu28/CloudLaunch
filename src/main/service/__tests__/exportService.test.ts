/**
 * @fileoverview exportService.tsのテスト（リファクタリング済み）
 *
 ...
 * - エクスポート統計の取得
 *
 * 注意: インポート機能は importService.test.ts に移行されました
 * 注意: ファイル解析機能は dataParserService.test.ts に移行されました
 */
...
describe("ExportService", () => {
  ...
  // importData メソッドのテストは importService.test.ts に移行されました

  // analyzeImportFile メソッドのテストは dataParserService.test.ts に移行されました
})
